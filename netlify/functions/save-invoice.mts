...
function buildProperties(p: InvoicePayload, isUpdate: boolean) {
  ...
  const typeLabel = p.type === "quotation" ? "Quotation" : p.type === "dp" ? "Invoice DP" : "Invoice Final Payment";
  ...
  if (p.metodePembayaran) props["Metode Pembayaran"] = { select: { name: p.metodePembayaran } };
...
export default async (req: Request, context: Context) => {
  ...
  try {
    ...
    if (existingId) {
      ...
      const sectionTitle = payload.type === "quotation" ? "Quotation" : payload.type === "dp" ? "Invoice DP" : "Final Payment Invoice";
